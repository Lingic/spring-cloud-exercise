    ...
    methods: {
      findPage:function(data){
        ...
        if(data != null){
          ...
          this.$api.white.findPage(Object.assign(this.pageRequest,{name:this.filters.name})).then((res) => {
      ...
      handleDelete: function (data) {
      	this.$api.white.batchDelete(data.params).then(data!=null?data.callback:'')
      },
      // 显示新增界面
      handleAdd: function() {
        ...
        this.$refs.dataForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              if(this.operation){
                this.$api.white.save(params).then((res) => {
                ...
                this.$api.white.edit(params).then((res) => {
                  this.editLoading = false
                  this.$message({
                    message: '提交成功',
                    type: 'success'
